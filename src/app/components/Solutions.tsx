...
export function Solutions() {
  ...
  return (
    <section id="solutions" className="py-16 md:py-24 bg-[#059669] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-10 md:mb-16">
          <motion.div 
            ...
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-white/90 text-[10px] font-black uppercase tracking-[0.2em] mb-4 md:mb-6 border border-white/20"
          >
            <Rocket size={12} />
            {t.solutionsBadge}
          </motion.div>
          <h2 className="text-3xl md:text-6xl font-black text-white mb-4 md:mb-6 leading-[1.1] tracking-tighter">
            {t.solutionsTitle}
            <span className="text-emerald-100">{t.solutionsTitleHighlight}</span>{t.solutionsTitle2}
          </h2>
          <p className="text-sm md:text-lg text-emerald-50/90 font-bold leading-relaxed max-w-2xl mx-auto">
            ...
            {solutions.map((item, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={`${item.span} min-w-[280px] md:min-w-0 snap-center group relative p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] bg-white/10 border border-white/20 hover:border-emerald-200/60 transition-all duration-500 overflow-hidden backdrop-blur-sm flex flex-col`}
                >
                  <div className="relative z-10 flex flex-col items-start h-full">
                    ...
                    <p className="text-sm md:text-lg text-emerald-50/80 leading-relaxed font-bold flex-grow">
                      {item.desc}
                    </p>
                  </div>
        ...
        <div className="mt-8 mb-16 relative -mx-4 px-4 sm:mx-0 sm:px-0">
          <motion.div
            ...
            viewport={{ once: true }}
            className="p-6 md:p-12 rounded-[2rem] md:rounded-[2.5rem] bg-white/10 border border-white/20 backdrop-blur-md"
          >
                ...
                <div className="grid grid-cols-4 gap-2 md:gap-12">
                   {steps.map((step, idx) => (
                      <div key={idx} className="flex flex-col items-center md:items-start text-center md:text-left group/step">
                         <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-white/15 border border-white/30 flex items-center justify-center text-white mb-2 md:mb-4 group-hover/step:bg-emerald-600 group-hover/step:border-emerald-500 transition-all duration-300">
                            <step.icon size={14} className="md:size-[18px]" />
                         </div>
                         <div className="text-[7px] md:text-[10px] font-black text-white/90 uppercase tracking-widest mb-0.5 md:mb-1">{step.number}</div>
                         <div className="text-[9px] md:text-xs font-black text-white uppercase tracking-tight leading-tight">{step.title}</div>
                         <div className="text-[7px] md:text-[9px] font-black text-white/60 uppercase tracking-widest mt-0.5 leading-none">{step.titleEn}</div>
                      </div>
                   ))}
                </div>
             </div>
        ...
        {/* Why Us? Badges */}
        <div className="mt-10 md:mt-16 bg-white/10 rounded-3xl md:rounded-[3rem] p-6 md:p-16 relative overflow-hidden border border-white/20">
          <div className="relative z-10">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8 mb-8 md:mb-12">
              <div className="text-center md:text-left">
                ...
                <div className="w-12 md:w-20 h-1 bg-white/70 rounded-full mx-auto md:mx-0" />
              </div>
              <p className="text-emerald-50/80 text-xs md:text-base font-medium max-w-md text-center md:text-right">
                {t.footerTaglineEn}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
              {differentiators.map((diff, i) => (
                <div key={i} className="bg-white/10 backdrop-blur-none border border-white/20 p-4 md:p-6 rounded-2xl hover:bg-white/15 transition-colors">
                  <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
                    <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-white/80 animate-pulse" />
                    <div className="text-white font-bold text-[10px] md:text-base">{diff.title}</div>
                  </div>
                  <div className="text-emerald-50/80 text-[9px] md:text-sm leading-relaxed font-medium">{diff.desc}</div>
                </div>
              ))}
            </div>
          </div>
